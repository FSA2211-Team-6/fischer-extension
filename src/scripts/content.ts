...
const getAssertionStats = async (id: number) => {
  try {
    const stats = await fetch(`https://faction-fischer.vercel.app/api/posts/${id}/stats`);
  ...
  if (parent) {
    element.addEventListener("mouseenter", () => {
      ...
      <div style='display:flex;'>
        ...
        <a style='display:flex;padding:20px;align-items:center' href='https://faction-fischer.vercel.app/posts/${
          post ? post.id : 1
...
const getExistingAssertions = async () => {
  ...
  try {
    const assertions = await fetch("https://faction-fischer.vercel.app/api/postURL", {
      method: "POST",
      body: urlJSON,
    });
